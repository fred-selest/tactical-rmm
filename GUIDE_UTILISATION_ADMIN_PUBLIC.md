...
Email address: admin@rmm.votre-domaine.com
...
Allez sur : **https://api.rmm.votre-domaine.com/admin/**
...
| Champ | Valeur pour rmm.votre-domaine.com |
|-------|------------------------------|
| **API URL** | `https://api.rmm.votre-domaine.com` |
...
1. Connectez-vous au dashboard : **https://rmm.votre-domaine.com**
2. Allez dans **Clients** dans le menu de gauche
3. Cliquez sur le client souhaité
4. Dans l'URL, vous verrez : `https://rmm.votre-domaine.com/clients/{CLIENT_ID}/sites/{SITE_ID}`
   - Notez le `CLIENT_ID`
   - Notez le `SITE_ID`
...
curl -H "Authorization: Token VOTRE_TOKEN" \
  https://api.rmm.votre-domaine.com/api/v3/clients/

# Lister les sites d'un client
curl -H "Authorization: Token VOTRE_TOKEN" \
  https://api.rmm.votre-domaine.com/api/v3/clients/{CLIENT_ID}/sites/
```
...
4. L'URL sera du format :
   ```
   https://mesh.rmm.votre-domaine.com/meshagents?id=XXXXXXXXXXXXXXXXXXXXXXX...
   ```
...
```
https://api.rmm.votre-domaine.com/clients/a1b2c3d4-e5f6-7890-abcd-ef1234567890/deploy/linux/
```
...
wget https://api.rmm.votre-domaine.com/clients/a1b2c3d4-e5f6-7890-abcd-ef1234567890/deploy/linux/ -O install-rmm.sh
...
1. Allez sur **https://rmm.votre-domaine.com**
...
1. Dans l'admin Django : **https://api.rmm.votre-domaine.com/admin/**
...
│                                             │
│ 🔗 Download URL:                            │
│   https://api.rmm.votre-domaine.com/clients/...  │
└─────────────────────────────────────────────┘
```
...
```bash
curl -H "Authorization: Token VOTRE_TOKEN" \
  https://api.rmm.votre-domaine.com/api/v3/linux-deployments/stats/ | jq
```
...
- ✅ Activer **HTTPS** (déjà fait sur rmm.votre-domaine.com)
