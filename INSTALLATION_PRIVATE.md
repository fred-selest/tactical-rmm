# 🚀 Guide d'Installation - Dashboard Linux pour rmm.selest.info
...
Cette installation va ajouter à votre interface **rmm.selest.info** :
...
- ✅ Le domaine **api.rmm.selest.info** est accessible
...
# ou
ssh votre-utilisateur@rmm.selest.info
sudo su -
```
...
```bash
curl -I https://api.rmm.selest.info/api/v3/linux-deployments/
```
...
```
https://api.rmm.selest.info/admin/
```
...
| **API URL** | `https://api.rmm.selest.info` | URL de votre API |
| **Auth Key** | `[votre clé]` | Clé d'authentification Tactical RMM |
| **Mesh URL** | `https://mesh.rmm.selest.info/...` | URL MeshCentral (optionnel) |
...
deployment = LinuxDeployment.objects.create(
    ...
    agent_type="server",
    arch="amd64",
    api_url="https://api.rmm.selest.info",
    mesh_url="https://mesh.rmm.selest.info/meshagents?id=VOTRE_MESH_ID",
...
print(f"URL: https://api.rmm.selest.info/clients/{deployment.uuid}/deploy/linux/")
...
```bash
curl -X POST https://api.rmm.selest.info/api/v3/linux-deployments/create/ \
...
```json
{
  ...
  "download_url": "https://api.rmm.selest.info/clients/a1b2c3d4.../deploy/linux/",
...
```bash
curl https://api.rmm.selest.info/api/v3/linux-deployments/ \
  -H "Authorization: Token VOTRE_TOKEN_API"
```
...
```bash
curl https://api.rmm.selest.info/api/v3/linux-deployments/stats/ \
  -H "Authorization: Token VOTRE_TOKEN_API"
```
...
wget https://api.rmm.selest.info/clients/{UUID}/deploy/linux/ -O install.sh
...
- ✅ **HTTPS obligatoire** (déjà configuré sur rmm.selest.info)
...
DEPLOY_UUID=$(curl -s -X POST https://api.rmm.selest.info/api/v3/linux-deployments/create/ \
...
echo "URL : https://api.rmm.selest.info/clients/$DEPLOY_UUID/deploy/linux/"

# 2. Distribuer sur vos 10 serveurs
for SERVER in server{1..10}; do
  ssh $SERVER "wget https://api.rmm.selest.info/clients/$DEPLOY_UUID/deploy/linux/ -O /tmp/install.sh && chmod +x /tmp/install.sh && sudo /tmp/install.sh"
done

# 3. Vérifier les installations
curl https://api.rmm.selest.info/api/v3/linux-deployments/$DEPLOY_UUID/ \
  -H "Authorization: Token VOTRE_TOKEN" \
  | jq '.install_count'
```
...
```bash
# Via API
curl "https://api.rmm.selest.info/api/v3/linux-deployments/?client_id=1" \
  -H "Authorization: Token VOTRE_TOKEN"

curl "https://api.rmm.selest.info/api/v3/linux-deployments/?site_id=2" \
  -H "Authorization: Token VOTRE_TOKEN"
```
